...
export default function BilettiEventCard({ event }: { event: BilettiEvent }) {
  return (
    <div className="group flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#0f0f1a] transition-all duration-300 hover:border-orange-500/30 hover:shadow-lg hover:shadow-orange-500/5 sm:flex-row">
      {event.imageUrl ? (
      ...
      <div className="flex flex-grow flex-col gap-4 p-6 sm:flex-row sm:items-center">
        <div className="flex shrink-0 flex-col items-center justify-center rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-center sm:min-w-[88px]">
        ...
        <div className="min-w-0 flex-grow">
          <h3
            className="text-lg font-bold text-white group-hover:text-orange-200 transition-colors"
            style={{ fontFamily: 'var(--font-display)' }}
          >
            {event.name}
          </h3>
          {event.description ? (
            <p className="mt-2 line-clamp-3 text-sm text-gray-400">{event.description}</p>
          ) : null}
        </div>
